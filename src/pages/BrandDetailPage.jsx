...
export default function BrandDetailPage() {
  const { id } = useParams();
  const brand = DataService.getBrand(id);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="absolute inset-0 flex items-center px-8 md:px-12">
          <div className="flex items-center gap-4">
            ...
            <div className="text-white">
              ...
              <p className="text-white/80 mt-1">{brand.category} · Est. {brand.founded} · {brand.productCount} Products</p>
            </div>
          </div>
        </div>
      </div>

      <p className="text-gray-600 text-lg mb-8 max-w-2xl">{brand.description}</p>
      ...
      <section className="mb-12">
        ...
        {products.length === 0 && <p className="text-gray-400 text-center py-12">No products available yet.</p>}
      </section>
